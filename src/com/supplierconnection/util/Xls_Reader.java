...
/*public static  int  gettotalcolumnofTCID(String testCase, String sheetname, Xls_Reader xls) {
	int testCaseStartRowNum= getcurrentRowNumofTCID(testCase, sheetname, xls);
	int testCaseStartColNum = 0;
	//int xCols = xls.getColumnCount(sheetname);
	//Getting starting column number
	for(int )
	for(int xCols=0;xCols<xls.getColumnCount(sheetname);xCols++){
		if(testCase.equals(xls.getCellData(sheetname, xCols, testCaseStartRowNum))){
			 testCaseStartColNum = xCols;
			break;
		}
		
	}
	return testCaseStartColNum;
	}*/
	
public int getColumnCountofaRow(String sheetName, int testCaseStartRowNum, Xls_Reader xls){
	// check if sheet exists
