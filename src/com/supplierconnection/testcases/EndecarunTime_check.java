...
public class EndecarunTime_check {
	...
	public void Businessprofiles(Hashtable<String, String> data) throws IOException{
		...
		if(!TestUtil.isTestCaseExecutable("EndecarunTime_check", xls))
			throw new SkipException("Flag set to no for testcase"+"EndecarunTime_check");
		
		//check the runmode of testdata
		if(data.get("RunMode").equals("N"))
			throw new SkipException("Flag set to no");
		
		k.executeKeywords("EndecarunTime_check", xls, data);
...
public  Object[][] getData(){
	...
	return TestUtil.getData("EndecarunTime_check", xls);
