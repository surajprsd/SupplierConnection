...
public class SupplierAcademy {
	...
	public void SupplierAcademy1(Hashtable<String, String> data) throws IOException{
		
		if(!TestUtil.isTestCaseExecutable("SupplierAcademy", xls))
			throw new SkipException("Flag set to no for testcase"+"SupplierAcademy");
		
		//check the runmode of testdata
		if(data.get("RunMode").equals("N"))
			throw new SkipException("Flag set to no");
		
		k.executeKeywords("SupplierAcademy", xls, data);	
...
public  Object[][] getData(){
	...
	return TestUtil.getData("SupplierAcademy", xls);
