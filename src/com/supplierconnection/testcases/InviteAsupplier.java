...
public class InviteAsupplier {
	...
	public void InviteAsupplier(Hashtable<String, String> data) throws IOException{
		
		if(!TestUtil.isTestCaseExecutable("InviteAsupplier", xls))
			throw new SkipException("Flag set to no for testcase"+"InviteAsupplier");
		
		//check the runmode of testdata
		if(data.get("RunMode").equals("N"))
			throw new SkipException("Flag set to no");
		
		k.executeKeywords("InviteAsupplier", xls, data);
...
public  Object[][] getData(){
	...
	return TestUtil.getData("InviteAsupplier", xls);
