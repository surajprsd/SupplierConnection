...
public class Announcements {
	...
	public void Announcements1(Hashtable<String, String> data) throws IOException{
		
		if(!TestUtil.isTestCaseExecutable("Announcements", xls))
			throw new SkipException("Flag set to no for testcase"+"Announcements");
		
		//check the runmode of testdata
		if(data.get("RunMode").equals("N"))
			throw new SkipException("Flag set to no");
		
		k.executeKeywords("Announcements", xls, data);
...
public  Object[][] getData(){
	...
	return TestUtil.getData("Announcements", xls);
